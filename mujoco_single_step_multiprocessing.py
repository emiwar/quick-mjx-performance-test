...
def time_multicore(batch_per_worker=32, n_workers=1, control_size=21):
    ...
    with multiprocessing.Pool(n_workers, load_model_to_global, (batch_per_worker,)) as p:
        for t in range(100):
            ...
            obs = np.stack(obs)
            if obs.shape != (n_workers, batch_per_worker, 27+28):#74+73):
                print(f"ERROR: obs.shape = {obs.shape}")
                break
            output_cuda = torch.from_numpy(obs)#.cuda()
    running_time = time.time()-s_time
    steps_per_s = 100 * batch_per_worker * n_workers / running_time
    ...
    return (batch_per_worker, running_time, steps_per_s)
    
if __name__ == '__main__':
    n_workers = 32
    results = []
    for batch_per_worker in (1,2,4,8,16,32,64,128,256,512):
        results.append(time_multicore(batch_per_worker, n_workers=n_workers))
    ...
    results_df['n_workers'] = n_workers
    results_df.to_csv('results/synchronized_multiprocessing_humanoid_mjx_32_cores.csv')
